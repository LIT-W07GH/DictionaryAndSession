...
namespace DictionaryAndSession.Controllers
{
    ...
    public class SessionDemoController : Controller
    {
        ...
        {
            int count = 0;
            if (HttpContext.Session.Get<MyClass>("counter") != null)
            {
                MyClass mc2 = HttpContext.Session.Get<MyClass>("counter");
                count = mc2.Counter;
            }
            ...
            HttpContext.Session.Set("counter", mc);
 ...
 * to that list. Then, when
 * the home page is shown, for every listing we're showing, if that listing is in 
